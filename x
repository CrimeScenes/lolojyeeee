...
local function UpdateFOV()
    ...
    local success, errorMsg = pcall(function()
        if Circle then
            Circle.Visible = getgenv().forbidden.PointLock.ProximityLimit_Visibility  -- Updated to new structure
            Circle.Radius = getgenv().forbidden.PointLock.ProximityLimit  -- Updated to new structure
...
local function GetTarget()
    if getgenv().forbidden.PointLock.Mode == "target" then  -- Updated to new structure
        return TargetPlayer  
    elseif getgenv().forbidden.PointLock.Mode == "normal" then  -- Updated to new structure
...
Mouse.KeyDown:Connect(function(Key)
    if Key:lower() == getgenv().Target.Keybind:lower() then
        if getgenv().forbidden.PointLock.Enabled then  -- Updated to new structure
            if IsTargeting then
...
RunService.RenderStepped:Connect(function()
    if IsTargeting and TargetPlayer and TargetPlayer.Character then
        ...
        local BodyPart
        if getgenv().forbidden.PointLock.ClosestPart then  -- Updated to new structure
            BodyPart = GetClosestBodyPart(TargetPlayer.Character)
        else
            BodyPart = TargetPlayer.Character:FindFirstChild(getgenv().forbidden.PointLock.HitPart)  -- Ensure this key exists in the table
        end

        if BodyPart then
            local predictedPosition
            if getgenv().forbidden.PointLock.Align then  -- Update this to the correct field in the new structure
                local humanoid = TargetPlayer.Character:FindFirstChildOfClass("Humanoid")
                if humanoid then
                    local moveDirection = humanoid.MoveDirection
                    predictedPosition = BodyPart.Position + (moveDirection * getgenv().forbidden.PointLock.Prediction)  -- Updated to new structure
                end
            else
                local targetVelocity = TargetPlayer.Character.HumanoidRootPart.Velocity
                predictedPosition = BodyPart.Position + (targetVelocity * getgenv().forbidden.PointLock.Prediction)  -- Updated to new structure
            end

            if predictedPosition then
                local DesiredCFrame = CFrame.new(Camera.CFrame.Position, predictedPosition)

                if getgenv().forbidden.PointLock.SmoothnessEnabled then  -- Updated to new structure
                    Camera.CFrame = Camera.CFrame:Lerp(DesiredCFrame, getgenv().forbidden.PointLock.Smoothness)  -- Updated to new structure
                else
